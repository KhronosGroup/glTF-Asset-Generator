...
+ Aim to have fewer than thirteen models and fewer than six properties being tested in a single model group.
...
+ Clearly identify models that are designed to fail to load. (Negative tests) 
  + A client that is compliant with the glTF 2.0 spec is expected to successfully render any model created by the glTF Asset Generator, unless explicitly noted otherwise. 
