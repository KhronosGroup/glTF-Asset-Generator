...
+ Code is as readable as possible. 
  ...
  + Don't remove properties when creating a model. We'd prefer properties to only be added to reduce potential confusion.

+ Aim to have fewer than thirteen models and less than about six properties being tested in a single model group.
...
+ Clearly identify models that are designed to fail to load. (Negative tests) 
  ...
  + If a model (or entire group of models) are not expected to load on a compliant client, then the model(s) must be marked as such in the readme and manifest.
