...
The following is a guide on how to add a new test as well as setting some expectations to help keep everything easy to read and consistent.
...
++ Having too many models in a single model group makes that model group difficult to consume manually, and causes too much scrolling to be needed to view the entire model group's readme. Consider splitting the model group into two or more that are closely related.
...
++ Any client that is compliant with the glTF 2.0 spec is expected to successfully render any model created by the glTF Asset Generator, unless explicitly noted otherwise. 
++ If a model (or entire group of models) are expected to not load on a complaint client, then the model(s) must be flagged as such in the readme and file manifest.

+ Typical Model Group
++ The first model has no test values set, and is intended to be used as a control duriung testing.
++ The last model has all of the test values set (as possible), and is intended as a quick test of having combinations of properties set.
...
1. Create a Model Group Readme Template in Markdown.
2. Create a new Model Group Class.
3. Delete existing models and screenshots in the output folder if needed. 
4. Compile and run the build.
5. Generate screenshots.

## Create a Model Group Readme Template in Markdown
Every model group needs a readme. The code automatically generates a table, but the template needs to be created beforehand with some details added manually.

1. Create a .md file named after the model group (same name as the class) to the `ReadmeTemplates` folder.
...
3. Optionally add `~~HeaderTable~~`. This text will be replaced with a table of properties that are present but not being actively tested.
...
+ Property names and values as listed in the readme are formatted by the `ReadmeStringHelper` class, which adds spaces, fixes capitalization, and converts values into strings. 
...
### Figures
Use images to better explain how a model is setup. This is especially useful for showing model properties that aren't nessesarry visible, like UV corridenents or joint skeletons.

1. Create the image
Use Draw.io to create figures. Export as a PNG, zoom 100%, selection only, Crop.
Save the project as an xml for use as a template in the Resources folder.
2. Add the files to the project
Place the PNG in .\Resources\Figures\ and set the file properties to "Copy if newer" (Visual Code?)
Place the template in .\Resources\Templates\
...
+ Clamp the size of the image to keep the image from changing cell widths and heights too much `<img src="Figures/BigImage.png" width="144" height="144" align="middle">`

## Create a new Model Group Class
1. Create a copy of the `ModelGroupTemplate` and place it under the `ModelGroups` folder.
2. Name the Class and .cs file as appropriate for what is being tested. For related model groups, use a CATEGORY_NAME format.
3. Add the name of the model group to the `ModelGroupId` enum in `ModelGroup.cs` (in alphabetical order). Then in the new class set the `Id` as that enum.
4. Call the class from `Program.cs`. This is done by creating an instance of the class to add to `allModelGroups` (in alphabetical order). 
...
+ Add the figures being used to the `UseFigure` list (See Figures above)
+ Declare the textures being used and add them to the `UseTexture` list. `var TextureImage = UseTexture(imageList, "FILENAME")`
+ Declare camera position(s) if a custome one will be used.
+ Deckare property values that will be used in more than one model. Add properties used by every model to the `CommonProperties` list.
...
+ Assemble and return the model, creating a new `Model` object using the component modle objects.

Add helper functions that reduce duplicate code below `CreateModel`. 
+ Generally this is where the properties list is populated, which is used in creating the model group readme (See Properties below)
...
For each tested property that is set on a model, a `Property` object needs to be created. 
```
...
Add new `PropertyName` enums in Property.cs alpabetically. 
If having a property name doesn't make sense, use `Description` as the enum and use a explanitory string as the value.

## Post Runtime Changes
There are some specific types of models that the Runtime layer isn't setup to create. For these cases there is an option to make post Runtime tweaks, specifically for cases when it doesn't make sense to make the changes to the Runtime code to do the same thing.
...
Setting a custom camera translation for a model group involves just setting the Camera object when creating a new model, in the `CreateModel` funtion.
```
return new Model
{
...
New base models are created in order to reduce duplicate code and to help focus model groups on the properties that are specificly being tested.
It is also a bad practice to remove properties from a model, as we'd prefer properties to only be added to reduce potential confusion.

Steps to add a new base model:
1. Create an abstract partial class of `ModelGroup`. Preface the name of the file with `ModelGroup_` to show this relation.
2. Create a statis partial class of the lowest level component model object. Typically this is `MeshPrimitive`. This should not create the entire glTF object if possible.
