...
$tempsourceSampleImageFolder = Join-Path -Path $tempFolderFromChild -ChildPath "SampleImages"
...
Get-ChildItem $tempImagesFolder -File | Foreach-Object {
    $imageExists = Join-Path -Path $sourceFolder -ChildPath "Resources" | Join-Path -ChildPath "Figures" | Join-Path -ChildPath "SampleImages" | Join-Path -ChildPath $_.Name
        ...
        if ($model.sampleImageName -ne $Null)
        {
            ...
            if ($existingImageList.Count -gt 0)
            {
                foreach($existingImage in $existingImageList){
                    if ($existingImage -eq (Split-Path $model.sampleImageName -Leaf))
                    {
                        $override = $True
                    }
                }
            ...
            elseif ((Test-Path $sourceSampleImage))
            {
