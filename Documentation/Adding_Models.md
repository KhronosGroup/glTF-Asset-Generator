...
2. Run the build with the argument `GenerateScreenshots`
    + This can be done from the debugger.
      + In VS Code open the debug panel.
      + Select the gear to edit the [launch.json](../.vscode/launch.json) 
      + Replace `"args": [],` with `"args": ["GenerateScreenshots"],`

Screenshots are not generated by default. This is done to speed up debugging.  
The creation of screenshots is a time intensive process and often the screenshots are not needed until the majority of debugging has been completed.
